...
#SBATCH --output=slurm_deepcoder_3b_16k-all_dataset_out_%j.txt
#SBATCH --error=slurm_deepcoder_3b_16k-all_dataset_error_%j.txt
#SBATCH --job-name=rllm-deepcoder-3b-16k

echo "Starting RLLM DeepCoder 3B 16K training job"
...
echo "SLURM output will be written to: slurm_deepcoder_3b_16k-all_dataset_out_${SLURM_JOB_ID}.txt"
echo "SLURM errors will be written to: slurm_deepcoder_3b_16k-all_dataset_error_${SLURM_JOB_ID}.txt"
...
$PYTHON_PATH -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name="qwen-3b-16k-grpo-code-all_dataset-job${SLURM_JOB_ID}" \
    ...
    trainer.default_local_dir="checkpoints/deepscaler/qwen-3b-16k-grpo-code-all_dataset-job${SLURM_JOB_ID}" \
